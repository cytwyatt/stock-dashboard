...
test('chat 迁移保留旧消息和合法时间戳，并移除无法显示的 createdAt', (t) => {
  const dataDir = tempDir(t);
  const baseJsonFile = jsonFile(dataDir);
  const file = path.join(dataDir, 'chats.json');
  baseJsonFile.writeDataJSON(file, {
    sessions: [{
      ...
      messages: [
        ...
        { role: 'assistant', content: 'valid', createdAt: 1700000000000 },
        { role: 'user', content: 'string', createdAt: '1700000000000' },
        { role: 'assistant', content: 'negative', createdAt: -1 },
        { role: 'user', content: 'null', createdAt: null },
        { role: 'assistant', content: 'object', createdAt: {} },
        { role: 'user', content: 'too-large', createdAt: 8640000000000001 },
      ],
    }],
  });
  ...
  store.migrateChatSessionIds();
  assert.equal(writes, 1);
  assert.deepEqual(store.readChats().sessions[0].messages, [
    { role: 'user', content: 'legacy' },
    { role: 'assistant', content: 'valid', createdAt: 1700000000000 },
