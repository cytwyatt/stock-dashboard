...
test('prepare 完成请求体、sessionId、stockContext 与消息的全部 400 校验', () => {
  ...
  assert.equal(prepared.userCreatedAt, 1000);
});
...
test('run 先持久化用户消息，并行自动证据，再按系统消息与最近12条历史调用模型', async () => {
  ...
  assert.deepEqual(h.writes[0].sessions[0].messages.at(-1), {
    role: 'user', content: '分析一下', createdAt: 1000,
  });
  assert.equal(h.writes[0].sessions[0].updatedAt, 1000);
  assert.deepEqual(h.writes[1].sessions[0].messages.at(-1), {
    role: 'assistant', content: '完成', createdAt: 1001,
  });
  assert.equal(h.writes[1].sessions[0].updatedAt, 1001);
  ...
  assert.equal(Object.hasOwn(messages.at(-1), 'createdAt'), false);
  assert.deepEqual(events.map((event) => event.type), ['tool', 'tool', 'answer']);
  assert.deepEqual(events.at(-1), {
    type: 'answer', content: '完成', createdAt: 1001, sessionId: 's1', title: '分析一下',
  });
});

test('用户消息时间在进入同会话队列前确定，不会被前一轮模型耗时推迟', async () => {
  const h = harness();
  let release;
  const gate = new Promise((resolve) => { release = resolve; });
  const blocker = h.service.withSessionLock('s1', () => gate);
  const prepared = h.service.prepare({ sessionId: 's1', message: '排队消息' });
  h.service.prepare({ sessionId: 'unused', message: '只用于推进测试时钟' });
  const events = [];
  const running = h.service.run(prepared, (event) => events.push(event));
  ...
  assert.deepEqual(h.getState().sessions[0].messages, [
    { role: 'user', content: '排队消息', createdAt: 1000 },
    { role: 'assistant', content: '完成', createdAt: 1002 },
  ]);
  assert.equal(events.at(-1).createdAt, 1002);
});
...
test('官方 DeepSeek V4 对复杂个股研究先非思考取数，再以无工具思考生成最终答案', async () => {
  ...
  assert.equal(events.at(-1).createdAt, 1001);
});
...
test('模型异常保留已写入的用户消息、输出 error 且不写 assistant', async () => {
  ...
  assert.deepEqual(h.getState().sessions[0].messages, [{
    role: 'user', content: '问题', createdAt: 1000,
  }]);
