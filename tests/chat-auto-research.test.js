...
test('个股研究意图只覆盖投研问题，并在首次模型请求前自动注入研究卡', async () => {
  ...
  global.fetch = async (input, init = {}) => {
    ...
    if (url === 'https://api.deepseek.com/v1/chat/completions') {
      ...
      llmBodies.push(body);
      const adaptiveThinking = body.thinking?.type === 'enabled';
      return new NativeResponse(JSON.stringify({
        choices: [{
          finish_reason: 'stop',
          message: { role: 'assistant', content: adaptiveThinking ? 'MOCK_DEEP' : 'MOCK_OK' },
        }],
  ...
  try {
    ...
    assert.deepEqual(llmBodies[1].thinking, { type: 'enabled' });
    assert.equal(llmBodies[1].reasoning_effort, 'high');
    assert.equal(Object.hasOwn(llmBodies[1], 'tools'), false);
    assert.equal(Object.hasOwn(llmBodies[1], 'tool_choice'), false);
    assert.equal(Object.hasOwn(llmBodies[1], 'temperature'), false);
    ...
    assert.equal(llmBodies.length, 3);
    assert.deepEqual(llmBodies[2].thinking, { type: 'disabled' });
    assert.ok(!llmBodies[2].messages.some((message) =>
      message.role === 'system' && message.content.includes('服务器已自动计算本轮个股研究卡')));
