...
test('盘后复盘 v2 提示词固定综合研判与条件式展望契约', () => {
  ...
  assert.match(prompt, /可以直接引用.*数字/);
  assert.match(prompt, /analysis.*模型/);
  assert.match(prompt, /先给出.*观点/);
...
test('综合研判允许数字与明确方向，仍拒绝保证、概率、投资建议和无来源事件归因', () => {
  ...
  const directional = structuredClone(valid);
  directional.headline = '核心指数收涨1%，短期价格结构仍偏强。';
  directional.synthesisOutlook.summary = '核心指数收盘结构改善，且阶段价格趋势同向，未来1至5个交易日的基准判断是震荡偏强。';
  directional.synthesisOutlook.baseCase.text = '核心指数与阶段价格结构同向改善，量能与市场宽度也没有明显背离，短期倾向维持震荡偏强。';
  directional.synthesisOutlook.baseCase.conditions[0] = '核心指数站稳3200点';
  assert.doesNotThrow(() => parseMarketReview(JSON.stringify(directional), allowed));
  ...
  const invalidCases = [
    ...
    },
    {
      name: '数字化涨跌概率',
      mutate(payload) {
        payload.synthesisOutlook.summary = '核心指数结构改善，未来上涨概率70%。';
      },
      error: /概率|投资建议|承诺/,
    },
    {
      name: '综合研判无来源因果',
      mutate(payload) {
        payload.synthesisOutlook.integratedAssessment = '由于政策持续发力，指数与阶段表现已经形成明确结论，市场内部的价格、量能、宽度和行业风格都支持这一判断。核心指数趋势与多周期结构相互确认，市场风险也已明显下降，后续只需跟踪成交结构即可。';
      },
      error: /事件归因|新闻证据|来源/,
    },
    {
    ...
    {
      name: '情景 bias 非法',
      mutate(payload) { payload.synthesisOutlook.upsideScenario.bias = '看多'; },
      error: /bias 非法/,
    },
    {
      name: '成立与失效条件重复',
  ...
  const conciseSynthesis = structuredClone(valid);
  conciseSynthesis.synthesisOutlook.evidenceRefs.integratedAssessment = ['indices'];
  assert.doesNotThrow(() => parseMarketReview(JSON.stringify(conciseSynthesis), allowed));
});
...
test('综合研判单个字段不合规时只降级该字段并保留其余模型分析', async () => {
  const payload = JSON.parse(serviceCompletion());
  ...
  assert.deepEqual(review.data.generationMeta.synthesisFallbackFields, ['summary']);
  ...
  assert.equal(review.data.card.outlook.bias, payload.synthesisOutlook.baseCase.bias);
  assert.equal(
  ...
  const preservedBase = review.data.detail.synthesisOutlook.scenarios
    .find((scenario) => scenario.key === 'base');
  assert.equal(preservedBase.text, payload.synthesisOutlook.baseCase.text);
  assert.ok(review.data.dataWarnings.some((warning) => (
    warning.includes('summary') && warning.includes('其余模型分析已保留')
  )));
});

test('基准情景字段降级时仍根据价格趋势给出方向，不默认数据不足', async () => {
  const payload = JSON.parse(serviceCompletion());
  payload.synthesisOutlook.baseCase.bias = '看多';
  const { service } = createHarness({ completionText: JSON.stringify(payload) });

  const review = await service.ensureReview('cn');
  const base = review.data.detail.synthesisOutlook.scenarios
    .find((scenario) => scenario.key === 'base');

  assert.deepEqual(review.data.generationMeta.synthesisFallbackFields, ['baseCase']);
  assert.equal(base.bias, '震荡偏强');
...
test('盘后复盘 JSON 严格校验证据引用与事件归因引用', () => {
  ...
  technical.sections.risks = [{
    text: '指数收涨但量能与宽度没有同步改善，模型判断这种背离会削弱短期延续性。',
    claimType: 'analysis', evidenceRefs: ['indices', 'indexHistory'], citationRefs: [],
  }];
  assert.equal(
    parseMarketReview(JSON.stringify(technical), allowed).sections.risks[0].claimType,
    'analysis',
  );

  technical.sections.risks[0].text = '政策发力推动指数上涨。';
  assert.throws(
    () => parseMarketReview(JSON.stringify(technical), allowed),
    /事件归因|新闻证据|来源/,
  );
});

test('宽容模式剔除不合规明细，显著文本按字段局部降级', () => {
  const allowed = {
  ...
  invalidHeadline.headline = '核心指数上涨1%，建议立即买入。';
  ...
  assert.equal(withFallback.cardSummary, payload.cardSummary);
  assert.deepEqual(withFallback.prominentEvidenceRefs.headline, ['indices']);
  assert.ok(withFallback.validationWarnings.some((warning) => (
    warning.code === 'prominent_field_fallback' && warning.field === 'headline'
  )));
});

test('显著文本必须绑定对齐证据且允许展示行情数字', () => {
  const allowed = {
    allowedEvidenceRefs: ['indices', 'indexHistory', 'overview'],
    associationEvidenceRefs: ['indices', 'indexHistory'],
  };
  ...
  assert.equal(parseMarketReview(JSON.stringify(numeric), allowed).headline, numeric.headline);
});
...
test('生成复盘时安全剔除单条不合规明细，不重试模型且记录数据提示', async () => {
  ...
  assert.equal(review.data.status, 'ready');
  assert.equal(calls.llm, 1);
  assert.equal(review.data.card.summary, payload.cardSummary);
  assert.equal(review.data.generationMeta.prominentFallbackUsed, false);
