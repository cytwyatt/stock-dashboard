...
- **港股**：恒生指数、国企指数、恒生科技指数；港股涨跌幅榜（已过滤仙股/无量异动）
...
- **个股详情**：点击涨跌榜/板块领涨股/搜索结果打开弹窗——完整报价（换手、PE、PB、市值等）+ 分时/日K/周K/月K；任意个股可从弹窗带着代码和市场上下文「问 AI」
- **自选股**：独立 tab，详情弹窗点 ♡ 添加，A股/港股/美股混合展示；每只股票可点「问 AI」带着代码和市场上下文进入新问题；存服务器 `data/watchlist.json`，所有设备共享同一份（部署更新时勿覆盖 data/ 目录）
- **搜索**：顶栏搜索框，支持代码/中文/拼音/英文（腾讯 smartbox），覆盖 A股、港股和美股
...
| 数据 | 来源 |
|---|---|
| A股/港股指数 / 分时 / K线 | 腾讯行情 (qt.gtimg.cn / web.ifzq.gtimg.cn) |
| A股行业板块 | 腾讯 (proxy.finance.qq.com) |
| A股/港股涨跌榜 | 新浪财经 |
| 美股指数 / 分时 / K线 / 涨跌榜 | Yahoo Finance (query1.finance.yahoo.com) |
| 财经新闻 | 新浪财经滚动 |

均为免费公开接口，服务端做了 15s~10min 的缓存以减少请求（收盘后榜单最长 10 分钟）。数据可能有延迟，仅供参考，不构成投资建议。
...
- 指数列表：server.js 中 `CN_INDICES`（腾讯代码，如 `sh000905` 中证500）/ `HK_INDICES`（如 `hkHSI` 恒生指数）/ `US_INDICES`（Yahoo 代码，如 `^RUT` 罗素2000）
