...
AI 会按需调用行情工具获取指数、报价、K 线、分时、板块、涨跌榜、市场概况、新闻和搜索结果。从个股入口发起的会话会持久化股票代码；遇到明显异动或询问原因时，服务端会自动检索近期个股资讯，并要求模型区分直接证据与板块关联推断、附上发布时间和来源链接。会话存储在服务端，API Key 原文不会回传到浏览器（配置接口仅返回掩码）。
