...
| 榜单行业标签 | 三个市场的领涨、领跌个股均显示简要行业；A股与港股异步补齐，不阻塞价格榜单 |
| 专业图表 | 分时、日 K、周 K、月 K，包含成交量和 MA5 / MA10 / MA20 |
| 公司概况卡 | 个股详情异步展示所属行业、板块和主营业务，支持来源、缓存状态与长文本展开 |
| 个股研究卡 | 汇总 1 / 5 / 20 / 60 / 120 日复权收益、基准超额、52 周位置、波动率、最大回撤和量能 |
| AI 盘后复盘 | A股、港股、美股收盘后各生成一次结构化深度复盘；卡片展示概要与短期前瞻，详情覆盖跨数据综合研判、多周期表现、宽度与量能、行业/风格、新闻证据、风险及条件式情景 |
...
| 市场 | 指数与概况 | 个股与榜单 |
|---|---|---|
| A股 | 上证指数、深证成指、创业板指、沪深300、科创50；上涨/未上涨家数、涨跌停、沪深成交额及前一交易日可比值、行业板块 | 带简要行业的沪深北涨跌榜、搜索、报价详情、行业与主营业务、分时与前复权 K 线 |
| 港股 | 恒生指数、国企指数、恒生科技指数；腾讯恒生指数口径大市成交额及前一交易日可比值 | 带简要行业的港股涨跌榜、搜索、报价详情、行业与公司业务、分时与复权 K 线 |
| 美股 | 道琼斯、纳斯达克、标普500；VIX、美债、美元、黄金、原油、比特币 | 带 Industry / Sector 的 Yahoo 涨跌榜、搜索、报价详情、Sector / Industry 与公司简介、分时与公司行动调整 K 线 |
...
每个市场在收盘缓冲期后生成一次复盘，成功结果按“市场 + 交易日”写入 `data/market-reviews.json`；刷新页面、重启服务、当天更换模型或升级复盘结构都直接复用，不会重复调用模型。服务端以核心指数最后一根完整日线确定交易日，并在行情仍为旧缓存时延后生成；失败会进入 30 分钟退避，已有上一份复盘时继续展示旧卡片。A股复盘使用指数与多周期日线、上涨/未上涨家数、沪深成交额比较、行业板块、代表性涨跌样本和滚动新闻；港股加入大市成交额比较；美股加入三大指数、多周期日线、11 只行业 ETF 代理、VIX、美债、美元、黄金、原油、比特币和新闻。模型会在分项复盘之外综合收盘时可得信息，提炼主线、相互确认与背离，再给出未来一至五个交易日的基准、偏强、偏弱三种条件式情景；每种情景都有成立条件和失效信号，不提供目标价、数字概率、买卖或仓位建议。所有内容采用固定 JSON 结构，事件归因必须引用具体新闻来源；ETF、宏观代理和涨跌榜样本不会被冒充为全市场宽度。官方 DeepSeek V4 复盘请求会显式启用高强度思考与 JSON Output，输出上限为 32,768 token；其他 OpenAI 兼容服务使用 16,000 token 的保守上限。系统会记录完成原因、耗时、token 用量和服务端校验降级情况。
...
| A股行业板块与资金流 | 腾讯行情 |
| A股 / 港股涨跌榜 | 新浪财经；可见榜单的行业标签异步复用新浪公司资料 |
| A股个股候选相关资讯 | 新浪财经个股资讯 |
| A股 / 港股行业与主营业务 | 新浪财经公司资料 |
| 美股指数、报价、分时、复权 K 线与带行业涨跌榜 | Yahoo Finance |
| 美股 Sector、Industry 与公司简介 | Nasdaq 公司资料 |
| 财经新闻 | 新浪财经滚动 |

行情数据会根据类型缓存 15 秒至 10 分钟，完整公司资料缓存 24 小时，并在打开个股详情或异步补充 A/H 可见榜单行业时复用；榜单价格不会等待公司资料。A股资料若只取得行业或主营业务之一，会明确提示部分资料并在 5 分钟内重试，不会覆盖已有的完整缓存。每日盘后复盘成功生成后持久化复用，失败时按 30 分钟退避重试。上游异常时服务端会尽量返回带 stale 标记的旧缓存；核心指数或日线仍为旧缓存时不会固化当日复盘。免费公开接口可能限流、延迟或调整格式。
...
- AI 盘后复盘的 `偏强 / 中性 / 偏弱 / 分化` 仍只描述已完成交易日的盘面状态；独立的短期前瞻是基于收盘时可得信息构建、可验证且可失效的情景推演，不是涨跌概率、收益承诺或投资建议。港股和美股缺少真实全市场宽度时会降低证据质量并明确代理或样本覆盖边界。
