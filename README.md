...
| AI 大盘总结 | 在 A股、港股、美股页面按市场生成结论、广度/覆盖、主要驱动、风险和后续观察点，展示数据时间与缓存状态 |
...
每个市场页的大盘总结先由服务端汇总结构化行情，再要求模型输出固定字段；常规交易时段缓存 5 分钟、其他时段缓存 30 分钟，刷新失败时会展示带警告的旧总结。A股总结使用指数、上涨/未上涨家数、板块和代表性个股；港股使用指数和代表性个股；美股使用三大指数与 VIX、美债、美元、黄金、原油、比特币等宏观代理，不把代理指标冒充市场宽度。交易阶段会区分盘前、集合竞价、午休、常规交易与盘后，但只依据星期和当地时钟，不包含交易所节假日日历。
