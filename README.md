...
一个可自托管、零 npm 依赖的多市场行情看板。聚合指数、涨跌榜、分时与 K 线、行业板块、财经新闻和自选股，并通过 OpenAI 兼容接口提供 AI 大盘总结与带实时行情上下文的问答。
...
| AI 大盘总结 | 在 A股、港股、美股页面生成带时间尺度、证据质量和引用的盘面结论，区分真实广度、同步信号、市场风险与数据质量 |
| AI 行情问答 | LLM 通过工具调用获取实时行情；个股异动会自动检索候选相关资讯并要求附来源，可从任意个股详情或自选股携带上下文进入问答 |
...
| 市场 | 指数与概况 | 个股与榜单 |
|---|---|---|
| A股 | 上证指数、深证成指、创业板指、沪深300、科创50；上涨/未上涨家数、涨跌停、成交额、行业板块 | 沪深北涨跌榜、搜索、报价详情、分时与前复权 K 线 |
...
每个市场页的大盘总结先由服务端汇总结构化行情并计算指数一致性、覆盖率和可用的市场宽度，再要求模型按固定字段解释盘面；每条信号、风险和观察点都要引用输入组件，只有同期行情时不得写成事件因果。常规交易时段缓存 5 分钟、其他时段缓存 30 分钟，缓存按具体交易阶段隔离，刷新失败时会展示带警告的旧总结。A股总结使用指数、上涨/未上涨家数、板块和代表性个股；港股使用指数和代表性个股；美股使用三大指数与 VIX、美债、美元、黄金、原油、比特币等宏观代理，不把代理指标冒充市场宽度。美债收益率变化使用基点（bp），没有历史比较基准时不把绝对成交额描述为放量或缩量。交易阶段会区分盘前、集合竞价、午休、常规交易与盘后，但只依据星期和当地时钟，不包含交易所节假日日历。

AI 问答会按需调用行情工具获取指数、报价、K 线、研究卡、分时、板块、涨跌榜、市场概况、新闻和搜索结果。从个股入口发起的会话会持久化股票代码；询问综合分析、阶段表现、相对强弱、风险回撤、52 周位置、量能或操作倾向时，服务端会在第一次模型请求前自动附上同口径研究卡。遇到明显异动或询问原因时，服务端会自动检索近期个股资讯，并要求模型区分价格/风险观察与事件因果、区分直接证据与板块关联推断，并附上发布时间和来源链接。会话存储在服务端，API Key 原文不会回传到浏览器（配置接口仅返回掩码）。
...
行情数据会根据类型缓存 15 秒至 10 分钟；AI 大盘总结常规交易时段缓存 5 分钟、其他时段缓存 30 分钟。上游异常时服务端会尽量返回带 stale 标记的旧缓存。免费公开接口可能限流、延迟或调整格式。
...
- AI 大盘总结的 `偏强 / 中性 / 偏弱 / 分化` 只描述当前或最近交易时段的盘面状态，不代表后市预测；港股和美股缺少真实全市场宽度时会降低证据质量并显示为“数据覆盖”。
