...
| 个股研究 | 行业、主营业务、多周期收益、基准超额、52 周位置；个股多因子画像含中期动量、相对强弱、波动、Beta、回撤与量能，可查看自身历史分位和走势 |
| AI 盘后复盘 | 每个市场每个交易日生成一次；A股/美股额外展示可复算的偏多、偏空、反证和下个交易日触发条件，模型仍只给条件式的一至五日情景 |
...
个股详情的研究卡支持切换至“多因子分析”，展示价格与风险维度、自身历史分位和最近60期走势。价值、质量、成长、规模及同行排名尚未覆盖；分位不是综合评分或涨跌概率。口径见 [个股因子说明](docs/stock-factors.md)。
...
盘后复盘按“市场 + 交易日”持久化，刷新、重启或当日更换模型都不会重复调用。A股/美股 v3 复盘还会在模型调用前冻结规则信号输入；规则事实由服务端计算，模型只能在白名单 ID 内解释确认、背离和事件线索。它们是未经样本外验证的工程规则，不是综合评分或涨跌概率。详细口径见 [多空信号说明](docs/market-directional-signals.md)。AI 输出只作为条件式情景参考，不提供目标价、涨跌概率、买卖或仓位建议。
...
1. 在运行看板的同一服务器用户下安装官方 **Codex CLI**，通过 `codex login --device-auth` 登录。升级 CLI 后需重新验证协议与权限边界。
2. 在模型设置选择订阅模式，填写账号可用的 Codex 模型（默认 `gpt-5.6-luna`、低推理强度），可单独指定下一份复盘的模型。
3. “保存并检查登录 / 模型”只检查账号、模型和配置，不生成测试回答，也不检查剩余额度。检查通过后即可问答；启用模式后的下一份到期复盘也使用 Codex。
...
**费用和稳定性限制：** 与日常 Codex 共享订阅额度；看板不再设 50% 暂停线，但账号原生额度和服务端限流仍然有效。不自动回退到付费 API、购买额度或兑换重置；账户已有加购额度可能受其他客户端或看板调用影响。App Server 没有等价的每请求 `max_tokens` 硬限制；本适配器限制时间/输入/输出长度，并在返回 token 用量超标时拒绝保存，但不能撤销已消耗额度。官方仍将 App Server 标为实验性，不支持生产工作负载，不能承诺长期稳定。
...
- `data/` 保存自选股、AI 会话、每日复盘、长期信号快照和模型配置，已被 Git 忽略；部署和备份时请单独处理。
