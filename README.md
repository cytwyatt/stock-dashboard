...
| 市场 | 指数与概况 | 个股与榜单 |
|---|---|---|
| A股 | 上证指数、深证成指、创业板指、沪深300、科创50；上涨/未上涨家数、涨跌停、沪深成交额及前一交易日可比值、行业板块 | 沪深北涨跌榜、搜索、报价详情、行业与主营业务、分时与前复权 K 线 |
| 港股 | 恒生指数、国企指数、恒生科技指数；腾讯恒生指数口径大市成交额及前一交易日可比值 | 港股涨跌榜、搜索、报价详情、行业与公司业务、分时与复权 K 线 |
| 美股 | 道琼斯、纳斯达克、标普500；VIX、美债、美元、黄金、原油、比特币 | Yahoo 涨跌榜、搜索、报价详情、Sector / Industry 与公司简介、分时与公司行动调整 K 线 |
...
行情数据会根据类型缓存 15 秒至 10 分钟，完整公司资料缓存 24 小时且只在打开个股详情时加载；A股资料若只取得行业或主营业务之一，会明确提示部分资料并在 5 分钟内重试，不会覆盖已有的完整缓存。每日盘后复盘成功生成后持久化复用，失败时按 30 分钟退避重试。上游异常时服务端会尽量返回带 stale 标记的旧缓存；核心指数或日线仍为旧缓存时不会固化当日复盘。免费公开接口可能限流、延迟或调整格式。
