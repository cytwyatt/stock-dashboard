...
tar czf - --exclude='.claude' --exclude='.codex' --exclude='.agents' --exclude='.git' --exclude='.DS_Store' --exclude='data' . \
...
| 新浪 A股 F10 公司资料需分别取公司简介与申万行业页，港股资料页的“所属行业”可能为空；均为 GBK HTML、无正式 API/SLA | A股两页必须并行且允许单项降级，港股以公司业务兜底；目标表格结构消失时必须报异常。完整公司资料只通过 `marketService.profile()` 按代码缓存 24 小时，单项降级必须标记 `coverage.complete=false` 并按 5 分钟候选缓存重试；旧完整缓存不能被 partial 覆盖，需以 stale 返回。A/H 榜单行业只能在价格榜单渲染后，通过最多 20 只、全局并发 3 的批量服务异步复用 profile，不能塞进 rank loader 或预热任务 |
| Nasdaq `company-profile` JSON 可提供美股 Sector、Industry 与 CompanyDescription，但 ETF、指数及部分特殊股票可能缺字段 | `src/providers/nasdaq.js` 只负责该上游；明确成功但字段缺失时返回 `available=false`/部分资料，带错误消息或异常状态的空响应必须报错，不得猜测行业；业务文本清理标签与实体后限长 |
...
- **行情查询与缓存**：`src/services/market-service.js` 是共享行情查询、研究卡依赖、公司资料、滚动新闻的唯一缓存 key 和 TTL 定义层。HTTP 路由、LLM 工具、自动研究/报价证据和 `cache-warmer` 都必须调用 `marketService`，不得自行拼 key/TTL 或绕过它直接调用 `cached()`/`cachedEntry()`。榜单 key 的 `open/closed` 后缀用于隔离开闭市 TTL 和在途请求，不能合并；预热周期必须短于最短行情 TTL。完整公司资料按代码缓存 24 小时，在打开详情或异步补充 A/H 可见榜单行业时复用，但不加入预热；刷新候选与首次 partial 只缓存 5 分钟，过期完整资料遇到 partial 或刷新失败必须继续作为 stale 返回。榜单行业批量服务只能处理前端实际展示、去重后的最多 20 只，并以实例级共享队列限制跨请求并发，任何单股失败都不能让价格榜单失败。个股事件证据包因需保留独立抓取时间来判断 stale，继续由 `stockEventsService` 管理专用缓存。缓存底层仍需保留过期兜底、并发合并和统一 `{data,meta}`；过期兜底不能更新成功抓取时间。
...
- **个股多因子**：`src/domain/stock-factors.js` 是无 I/O 的价格与风险计算，通过原 `marketService.research()` 返回 `factorAnalysis`，不得增加上游查询或自行定义缓存。全体因子仅用完整日线；动量126→21区间、相对强弱/波动/Beta为60区间、回撤幅度120区间（正值）、相对量能20日。比较因子要求连续基准交易日有实际个股收盘，不跳过缺失或前填停牌收益；自身分位用此前最多120期、至少60个有效观察，排除当期，并列用中位秩，不是同行排名或预测概率。价值/质量/成长/规模/同行分位未覆盖，不能伪造或把相关因子合成评分；历史轨迹是当前复权日线回算，不是事前预测。图表由 `public/stock-factors.js` 渲染，沿用研究卡请求守卫；AI 自动证据和工具结果必须压缩掉因子历史数组，口径见 `docs/stock-factors.md`。
- **盘后复盘的一日一次语义**：`marketReviewService` 只能通过 `marketService` 采集行情，成功结果由 `marketReviewStore` 按“市场 + 核心指数完整交易日”写入 `data/market-reviews.json`，同日刷新、重启、更换模型或升级复盘 schema 都不得重算。A/H/美股分别在当地收盘后保留 10/15/15 分钟缓冲；核心指数最后一根完整日 K 是交易日与收盘 OHLC 的事实源，实时指数报价只用于枚举代码/名称，核心日线旧缓存或交易日尚未完整时不得落盘。并发请求要合并，失败记录 30 分钟退避，上一份成功复盘可继续展示但状态必须明确。定时检查独立于 5 秒行情预热，隔离测试用 `MARKET_DISABLE_REVIEW=1` 关闭。
- **前端竞态守卫**：图表加载用递增请求序号（`chartReq`/`modal.req`），公司资料和研究卡分别使用 `modal.profileReq` / `modal.researchReq`，榜单及其异步行业补充使用独立 `rankReq`；慢的旧响应直接丢弃。按市场加载的区块（指数/榜单/概况/板块）在 await 前捕获 `state.market`、响应回来后比对不一致就丢弃——快速切 tab 时旧市场的慢响应会后到（曾导致港股 tab 显示A股榜单）。A/H 行业只补实际显示的各 10 只，前端会话缓存和 in-flight 合并必须避免 30 秒轮询重复请求；`switchMarket` 还会立即清空榜单/概况，避免新数据到达前残留旧市场内容。新增异步渲染要沿用这两个模式。
...
- **Codex 订阅模式**：`transport=codex` 的问答/复盘模型分别存 `codexModel/codexMarketReviewModel`，与原 API 配置分离，`LLM_TRANSPORT` 可强制模式，`LLM_MODEL/LLM_MARKET_REVIEW_MODEL` 仍覆盖当前模式。`src/ai/codex-client.js` 使用服务器当前 CLI，不按版本号拒绝；兼容性通过私有 stdio 协议结构、官方路由、权限、登录与响应校验逐项把关。临时会话使用无环境访问的受控 JSON 动作，实际查询继续由原工具白名单与最多6轮循环执行。权限 profile 必须在子进程启动参数中定义（只传 thread/start 会在 turn/start 重载时丢失），关闭原生命令/插件/hooks/记忆/子代理；发现启用的外部 MCP 或非官方路由必须拒绝。只接受 ChatGPT 登录；不设本地订阅额度百分比阈值，仍受账号原生额度和服务端限流约束，绝不自动 API fallback、购买额度或兑换重置。Codex 无等价 max_tokens 硬限制，超时与完成后 token 超标校验不能撤销已用额度。连接检查不生成回答，真实联调只使用明确的测试数据；协议 mock、配置往返及复盘一日一次测试必须覆盖订阅模式。
...
- 配置在服务器 `data/llm.json`（页面「⚙ 模型设置」可改）。`model` 用于 AI 问答，可选 `marketReviewModel` 只用于下一份尚未生成的盘后复盘；新的官方 DeepSeek 配置推荐两者统一使用 `deepseek-v4-flash`，已有配置缺少复盘字段时继续沿用 `model`，避免静默迁移。环境变量 `LLM_MODEL` 保留全局覆盖语义，`LLM_MARKET_REVIEW_MODEL` 可再单独覆盖复盘。**API Key 永不回传前端**（GET 只给掩码）、永不进 git。
- 官方 DeepSeek V4 的简单问答与最多 6 轮工具采集显式关闭 thinking；仅当会话绑定个股且 `hasStockResearchIntent()` 命中时，工具阶段结束后才把自动研究/资讯与本轮工具结果重新封装为受控证据，追加一次无 tools、thinking high、无 temperature 的最终综合。不得把非思考 `tool_calls` 原样送进思考请求；只接受 `finish_reason=stop`、无工具调用且内容非空的终稿，超时、截断或格式异常必须回退普通答案，`reasoning_content` 永不持久化。其他兼容端点不得盲目发送 DeepSeek 专属参数。复盘仍显式开启 thinking high 和 JSON Output，`max_tokens=32768` 且超时 5 分钟；完成原因、耗时、token 用量和校验降级数量写入 `generationMeta`，非正常结束不能保存。
- 会话存 `data/chats.json`。新写入的 user/assistant 消息都必须携带毫秒 epoch `createdAt`，助手 SSE 事件复用同一个时间；前端按浏览器本地时区显示，旧消息缺失或非法时间时显示“历史消息”，不得用加载时间伪造。发送给模型的上下文仍只映射最近 12 条消息的 role/content，不携带时间或历史工具结果；每问最多 6 轮工具调用——这是成本护栏，放宽前先想清楚。
...
- 自动研究卡测试必须让 mock LLM 的工具阶段首轮直接返回普通答案，并断言第一次请求的 `messages` 已含研究卡 system 消息；不能只检查 tools 定义或让 mock 先返回 `tool_calls`，否则无法证明是服务端预注入。官方 DeepSeek V4 正向用例还必须断言随后只有一次无 tools 的 thinking 终稿请求，负向意图不得出现该请求。
