...
- 复盘 schema v2 的 `synthesisOutlook` 必须在分项事实之外提炼跨组件主线、相互确认、背离和脆弱点，并给出未来一至五个交易日的基准/偏强/偏弱三种条件式情景；期限由服务端固定注入，每种情景都要有不重复的成立条件、失效信号和收盘对齐的 `evidenceRefs`。综合研判必须包含指数趋势，并覆盖当前实际可用的宽度/流动性、行业/风格、跨资产证据类别；引用 `news` 时还必须带白名单 `citationRefs` 和明确来源归属。禁止任何数字概率、目标价、确定性涨跌、买卖、止盈止损、仓位或收益承诺。`stance` 仍只描述已完成交易日；前瞻证据质量最多为中、必须标明不是涨跌概率，fallback 时强制为低且 bias 为“数据不足”。模型输出不合规时只允许使用服务端条件式 fallback，并在 `generationMeta.synthesisFallbackUsed` 和数据提示中记录；历史 v1 复盘缺少新字段时前端直接兼容，不得触发重算。
...
- 配置在服务器 `data/llm.json`（页面「⚙ 模型设置」可改）。`model` 用于 AI 问答，可选 `marketReviewModel` 只用于下一份尚未生成的盘后复盘；新的官方 DeepSeek 配置推荐分别使用 `deepseek-v4-flash` / `deepseek-v4-pro`，已有配置缺少复盘字段时继续沿用 `model`，避免静默迁移。环境变量 `LLM_MODEL` 保留全局覆盖语义，`LLM_MARKET_REVIEW_MODEL` 可再单独覆盖复盘。**API Key 永不回传前端**（GET 只给掩码）、永不进 git。
