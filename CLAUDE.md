...
| 新浪 A股 F10 公司资料需分别取公司简介与申万行业页，港股资料页的“所属行业”可能为空；均为 GBK HTML、无正式 API/SLA | A股两页必须并行且允许单项降级，港股以公司业务兜底；目标表格结构消失时必须报异常。完整公司资料只通过 `marketService.profile()` 按代码缓存 24 小时，单项降级必须标记 `coverage.complete=false` 并按 5 分钟候选缓存重试；旧完整缓存不能被 partial 覆盖，需以 stale 返回。不得塞进 15 秒报价或预热任务 |
...
- **前端竞态守卫**：图表加载用递增请求序号（`chartReq`/`modal.req`），公司资料和研究卡分别使用 `modal.profileReq` / `modal.researchReq`，慢的旧响应直接丢弃；按市场加载的区块（指数/榜单/概况/板块）在 await 前捕获 `state.market`、响应回来后比对不一致就丢弃——快速切 tab 时旧市场的慢响应会后到（曾导致港股 tab 显示A股榜单）。`switchMarket` 还会立即清空榜单/概况，避免新数据到达前残留旧市场内容。新增异步渲染要沿用这两个模式。
