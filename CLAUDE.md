...
- 官方 DeepSeek V4 的普通问答显式关闭 thinking 以保持低延迟工具调用，复盘则显式开启 thinking high 和 JSON Output、省略无效的 temperature，`max_tokens=32768` 且超时 5 分钟；其他 OpenAI 兼容端点使用 16000 上限且不得盲目发送 DeepSeek 专属参数。完成原因、耗时、token 用量和校验降级数量写入复盘的 `generationMeta`，非正常 `finish_reason` 或空内容必须判失败，不能保存截断结果。
