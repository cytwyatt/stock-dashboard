...
- **盘后复盘的一日一次语义**：`marketReviewService` 只能通过 `marketService` 采集行情，成功结果由 `marketReviewStore` 按“市场 + 核心指数完整交易日”写入 `data/market-reviews.json`，同日刷新、重启或更换模型都不得重算。A/H/美股分别在当地收盘后保留 10/15/15 分钟缓冲；核心指数最后一根完整日 K 是交易日与收盘 OHLC 的事实源，实时指数报价只用于枚举代码/名称，核心日线旧缓存或交易日尚未完整时不得落盘。并发请求要合并，失败记录 30 分钟退避，上一份成功复盘可继续展示但状态必须明确。定时检查独立于 5 秒行情预热，隔离测试用 `MARKET_DISABLE_REVIEW=1` 关闭。
