...
const { compactFactorAnalysis } = require('./research-intent');
...
const LLM_TOOLS = [
  ...
  { name: 'get_research_card', description: '获取个股研究卡：1/5/20/60/120日复权收益、相对同市场价格指数的超额收益、52周位置、20日年化波动率、120日最大回撤和较20日均量，以及个股多因子（动量、相对强弱、波动、Beta、回撤幅度、量能）与自身历史分位；没有同行排名和财务因子。适合回答阶段表现、风险和相对强弱；超额为简单收益率百分点差，不是风险调整后Alpha。若meta.stale=true或signals提示旧缓存，回答必须明确数据日期和缓存状态', parameters: { type: 'object', properties: { code: { type: 'string' } }, required: ['code'] } },
  ...
  async function run(name, args = {}) {
    switch (name) {
      ...
      case 'get_research_card': {
        ...
        if (data.factorAnalysis) data.factorAnalysis = compactFactorAnalysis(data.factorAnalysis);
        return { data, meta: marketMeta(entry, { market: marketForCode(code) }) };
      }
      case 'get_intraday': {
        const code = sanitizeCode(args.code || '');
