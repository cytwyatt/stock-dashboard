...
- **缓存**：`cached(key, ttl, fn)` 带过期兜底和并发合并；路由用 `cachedEntry()` 将 `fetchedAt/stale/staleSince` 放进统一 `{data,meta}` 响应。过期兜底不能更新成功抓取时间。**缓存 key 和 TTL 必须与 `warmCaches()` 里的预热项保持一致**，否则预热白做。
...
- **研究卡口径**：缓存 key 为 `k:${code}:400:day`、TTL 5 分钟；以基准交易日历对齐，股票停牌日仅向前延用最近收盘，严禁未来填充，并用 `lastTradeDate/analysisAsOf` 显式区分。阶段收益严格需要 N+1 个点；基准历史短缺时个股指标仍用自身交易日序列，仅基准/超额降级。超额是简单收益率百分点差；52周必须覆盖365自然日；波动率、回撤和量能排除盘中未完成日。历史不足时返回 `null + reason`，不能补零。
