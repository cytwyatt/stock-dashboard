...
async function buildAutomaticStockResearch(stockContext, text, { onTool } = {}) {
  ...
  try {
    const entry = await getResearchCardEntry(stockContext.code);
    const result = {
      data: entry.data,
    ...
    console.error(`[stock-research] ${stockContext.code}: ${error.message}`);
    return {
...
async function handleChat(res, payload) {
  ...
  return withChatLock(sessionId, async () => {
    ...
    try {
      ...
      const onAutomaticTool = (name, args) => sseSend(res, { type: 'tool', name, args });
      const [evidenceResult, researchResult] = await Promise.allSettled([
        buildAutomaticStockEvidence(stockContext, text, {
          checkMove: !!incomingStockContext,
          onTool: onAutomaticTool,
        }),
        buildAutomaticStockResearch(stockContext, text, { onTool: onAutomaticTool }),
      ]);
      if (evidenceResult.status === 'rejected') console.error(`[stock-evidence] automatic: ${evidenceResult.reason?.message || evidenceResult.reason}`);
      if (researchResult.status === 'rejected') console.error(`[stock-research] automatic: ${researchResult.reason?.message || researchResult.reason}`);
      const automaticEvidence = evidenceResult.status === 'fulfilled' ? evidenceResult.value : null;
      const automaticResearch = researchResult.status === 'fulfilled' ? researchResult.value : null;

      // 上下文：系统提示 + 会话股票元数据 + 自动研究卡/资讯证据 + 最近12条历史。
      const convo = [
        { role: 'system', content: llmSystemPrompt() },
        ...(stockContext ? [{ role: 'system', content: stockContextSystemMessage(stockContext) }] : []),
        ...(automaticResearch ? [{ role: 'system', content: stockResearchSystemMessage(automaticResearch) }] : []),
